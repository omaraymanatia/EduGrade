...
import { insertExamSchema, insertQuestionSchema, insertOptionSchema, insertStudentExamSchema, insertStudentAnswerSchema, InsertExam } from "@shared/schema";
...
export function registerRoutes(app: Express): Server {
  ...
  app.post("/api/exams", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    if (req.user!.role !== "professor") return res.status(403).json({ message: "Only professors can create exams" });
  ...
  app.get("/api/exams/:id", async (req, res) => {
    ...
    try {
      ...
      if (req.user!.role === "professor" && exam.creatorId !== req.user!.id) {
        return res.status(403).json({ message: "You don't have access to this exam" });
      }
      ...
      res.json({
        ...
        studentResults: req.user!.role === "professor" ? studentResults : undefined
      });
    } catch (error) {
  ...
  app.patch("/api/exams/:id", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    if (req.user!.role !== "professor") return res.status(403).json({ message: "Only professors can update exams" });
    ...
    try {
      ...
      if (exam.creatorId !== req.user!.id) {
        return res.status(403).json({ message: "You don't have permission to update this exam" });
      }
  ...
  app.post("/api/verify-exam-key", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    if (req.user!.role !== "student") return res.status(403).json({ message: "Only students can access exams with keys" });
    ...
    try {
      ...
      if (!exam) {
        return res.status(404).json({ message: "Exam not found with the provided key" });
      }
  ...
  app.post("/api/start-exam", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    if (req.user!.role !== "student") return res.status(403).json({ message: "Only students can take exams" });
    ...
    try {
      ...
      const studentExams = await storage.getStudentExamsByStudent(req.user!.id);
      const existingAttempt = studentExams.find(se => 
        se.examId === examId && se.status === "in_progress"
      );
  ...
  app.post("/api/submit-answer", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    if (req.user!.role !== "student") return res.status(403).json({ message: "Only students can submit answers" });
    ...
    try {
      ...
      if (studentExam.studentId !== req.user!.id) {
        return res.status(403).json({ message: "This exam attempt doesn't belong to you" });
      }
      
      if (studentExam.status !== "in_progress") {
        return res.status(400).json({ message: "This exam attempt is already completed" });
      }
      
      // Get the question to check type
      const question = await storage.getQuestionsByExam(studentExam.examId)
  ...
  app.post("/api/complete-exam", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    if (req.user!.role !== "student") return res.status(403).json({ message: "Only students can complete exams" });
    ...
    try {
      ...
      const answers = await storage.getStudentAnswersByStudentExam(studentExamId);
      ...
      const updatedStudentExam = await storage.updateStudentExam(studentExamId, {
        status: "completed",
  ...
  app.post('/api/exams/upload', examPhotoUpload.array('examPhotos', 20), async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    if (req.user!.role !== 'professor') return res.status(403).json({ message: "Only professors can create exams" });
    ...
    try {
      ...
      const examData = {
        ...
        creatorId: req.user!.id,
        passingScore: 70,
        instructions: "This exam was automatically generated from uploaded photos.",
        isActive: true
      };
  ...
  app.post('/api/upload-student-answers', upload.array('images', 10), async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    if (req.user!.role !== 'professor') return res.status(403).json({ message: "Only professors can upload student answers" });
