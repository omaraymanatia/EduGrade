...
export default function ProfessorExamDetails() {
  ...
  const toggleActiveMutation = useMutation({
    mutationFn: async (isActive: boolean) => {
      const res = await apiRequest("PATCH", `/api/exams/${examId}`, { isActive });
      ...
      cell: (data: any) => (
        <div className="flex items-center">
          ...
          <div>
            ...
            <div className="text-sm text-muted-foreground">{data.student?.email}</div>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      cell: (data: any) => (
        data.completedAt && data.startedAt ? 
          calculateTimeTaken(new Date(data.startedAt), new Date(data.completedAt)) : 
          "-"
      ),
    },
    {
    ...
    },
    {
      ...
      cell: (data: any) => (
        data.status === "completed" ? (
          <Badge variant={data.score >= (examDetails?.passingScore || 0) ? "success" : "destructive"}>
            {data.score >= (examDetails?.passingScore || 0) ? "Passed" : "Failed"}
          </Badge>
        ) : (
          <Badge variant="outline">In progress</Badge>
        )
      ...
      <div className="container mx-auto px-6 py-8">
        <div className="flex items-center mb-6">
          ...
          {isLoading ? (
            ...
            <h3 className="text-gray-700 text-2xl font-medium">{examDetails?.title}</h3>
          )}
        </div>
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div>
                    ...
                    <p className="mt-1 text-lg font-medium text-gray-900">{examDetails?.courseCode}</p>
                  </div>
                  <div>
                    <h4 className="text-sm font-medium text-gray-500">Created</h4>
                    <p className="mt-1 text-lg font-medium text-gray-900">{formatDate(examDetails?.createdAt)}</p>
                  </div>
                  <div>
                    <h4 className="text-sm font-medium text-gray-500">Exam ID</h4>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-lg font-medium text-gray-900">{examDetails?.duration} minutes</p>
                  </div>
                  <div>
                    <h4 className="text-sm font-medium text-gray-500">Passing Score</h4>
                    <p className="mt-1 text-lg font-medium text-gray-900">{examDetails?.passingScore}%</p>
                  </div>
                  <div>
                    <h4 className="text-sm font-medium text-gray-500">Status</h4>
                    <div className="mt-1 flex items-center space-x-2">
                      <Badge variant={examDetails?.isActive ? "success" : "secondary"}>
              ...
              <Card>
                <CardContent className="p-6 text-center py-20">
                  ...
                  <p className="text-muted-foreground mt-1">
                    Share the exam key <code className="bg-gray-100 p-1 rounded">{examDetails?.examKey}</code> with your students.
                  </p>
                </CardContent>
              </Card>
            )}
          ...
          <TabsContent value="uploads" className="mt-4">
            {isLoading ? (
              ...
              <div className="space-y-6">
                ...
                <Card>
                  <CardHeader>
                    <CardTitle>Instructions for Uploading Student Answers</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      <div className="flex items-start">
                        <div className="bg-primary-50 h-8 w-8 rounded-full flex items-center justify-center text-primary-600 mr-3">1</div>
                        <div>
                          <h4 className="font-medium">Take Photos of Student Work</h4>
                          <p className="text-muted-foreground">
                            Take clear photos of student handwritten answers. Ensure good lighting and focus.
                          </p>
                        </div>
                      </div>
                      
                      <div className="flex items-start">
                        <div className="bg-primary-50 h-8 w-8 rounded-full flex items-center justify-center text-primary-600 mr-3">2</div>
                        <div>
                          <h4 className="font-medium">Upload Using the Form Above</h4>
                          <p className="text-muted-foreground">
                            Click "Add Images" to select multiple images at once, or drag and drop files into the upload area.
                          </p>
                        </div>
                      </div>
                      
                      <div className="flex items-start">
                        <div className="bg-primary-50 h-8 w-8 rounded-full flex items-center justify-center text-primary-600 mr-3">3</div>
                        <div>
                          <h4 className="font-medium">Grade the Answers</h4>
                          <p className="text-muted-foreground">
                            After uploading, you can assign scores to the student's answers in the platform.
                          </p>
                        </div>
                      </div>
                    </div>
              ...
              {examDetails?.questions?.map((question: any, index: number) => (
                <div key={question.id} className="border rounded-md p-4">
                  ...
                  {question.type === "multiple_choice" && question.options && (
                    <div className="space-y-2 mt-4">
                      {question.options.map((option: any) => (
                        <div key={option.id} className="flex items-center p-2 border rounded-md">
                          <input 
                            type="radio" 
